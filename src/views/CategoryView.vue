<template>
  <div class="max-w-6xl mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold mb-6 text-center">Product Categories</h1>

    <div class="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mb-8">
      <div
        v-for="category in categories"
        :key="category.id"
        class="cursor-pointer bg-white rounded-xl overflow-hidden shadow hover:shadow-lg transition"
    ...
    <div v-if="selectedCategory" class="mb-8">
      ...
      <div v-if="paginatedProducts.length">
        <table class="min-w-full bg-white rounded-md shadow overflow-hidden text-sm">
      ...
      <p v-else class="text-center text-gray-500 mt-6">No products found for this category.</p>
    </div>
  </div>
</template>
...
const fetchProducts = async () => {
  ...
  const { data } = await axios.get(
    `https://api.escuelajs.co/api/v1/categories/${selectedCategory.value.id}/products`,
    {
      params: {
        offset,
